import os
import requests


API_URL = "http://127.0.0.1:5000"

TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..EX4pwLzGzgRjLjUq22AUivgkYdqpM7a1uQhnP3uMkRQ"

IMAGE_FOLDER = "package_images"


headers = {
    "Authorization": f"Bearer {TOKEN}"
}


for filename in os.listdir(IMAGE_FOLDER):
    if not filename.lower().endswith(
        (".jpg", ".jpeg", ".png", ".webp")
    ):
        continue

    package_id = os.path.splitext(filename)[0]

    image_path = os.path.join(
        IMAGE_FOLDER,
        filename
    )

    print(
        f"\nUploading image for package {package_id}..."
    )

    # 1. Upload to Cloudinary through your API
    with open(image_path, "rb") as image_file:
        upload_response = requests.post(
            f"{API_URL}/api/admin/uploads/image",
            headers=headers,
            files={
                "image": image_file
            },
            data={
                "folder": "packages"
            },
        )

    if upload_response.status_code != 201:
        print(
            "❌ Upload failed:",
            upload_response.text
        )
        continue

    upload_data = upload_response.json()

    image_url = upload_data["url"]

    print(
        "✅ Cloudinary:",
        image_url
    )

    # 2. Update package
    update_response = requests.patch(
        f"{API_URL}/api/packages/{package_id}",
        headers={
            **headers,
            "Content-Type": "application/json",
        },
        json={
            "image": image_url
        },
    )

    if update_response.ok:
        print(
            f"✅ Package {package_id} updated"
        )
    else:
        print(
            f"❌ Package {package_id} update failed:",
            update_response.text
        )


print("\n🔥 Finished.")